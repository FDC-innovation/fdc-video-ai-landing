...
import { Scissors, Subtitles, Crop, Mic2, MonitorPlay, Music, Palette, PenTool, LayoutGrid, Download } from "lucide-react";
...
function TextCard({ title, desc, tag }: { title: string; desc: string; tag?: string }) {
  return (
    <div className="bg-white rounded-[32px] p-10 md:p-16 flex flex-col justify-center h-full shadow-[0_12px_40px_rgba(0,0,0,0.03)] border border-gray-100">
      ...
      <h3 className="text-[32px] md:text-[42px] font-semibold text-[#1C1C1C] mb-6 leading-[1.1] tracking-[-0.02em]" style={{ fontFamily: "var(--font-syne)" }}>
        {title}
      </h3>
      <p className="text-[#4B5563] text-[18px] md:text-[20px] leading-[1.6]" style={{ fontFamily: "var(--font-inter)" }}>
        {desc}
      </p>
    </div>
  );
...
function VisualCard({ children, bgClass }: { children: React.ReactNode; bgClass: string }) {
  return (
    <div className={`relative w-full min-h-[400px] md:min-h-[500px] rounded-[32px] overflow-hidden flex items-center justify-center p-8 ${bgClass}`}>
      {children}
    </div>
  );
}
    ...
    <section className="bg-[#F4F4F5] py-24 px-6 md:px-12 w-full font-sans border-b border-gray-100">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="mb-20 text-center max-w-3xl mx-auto">
          <p className="text-[#5A6BFB] font-bold text-sm tracking-widest uppercase mb-4" style={{ fontFamily: "var(--font-inter)" }}>Capabilities</p>
          <h2 className="text-[#1C1C1C] text-[40px] md:text-[64px] font-[900] leading-[1.1] tracking-[-0.02em] mb-6" style={{ fontFamily: "var(--font-inter)" }}>
            Everything you need.<br/> Nothing you don&apos;t.
          </h2>
          <p className="text-[#3F3F46] text-xl md:text-2xl font-medium tracking-[-0.01em]" style={{ fontFamily: "var(--font-inter)" }}>
        ...
        <div className="flex flex-col gap-6 mb-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <VisualCard bgClass="bg-[#EEF2FF] p-0 block relative">
              <div className="absolute inset-0 w-full h-full">
                <img src="/photo/ChatGPT%20Image%20Apr%2015,%202026,%2010_35_17%20AM.png" alt="AI Smart Edit" className="w-full h-full object-contain p-8 md:p-12 scale-110" />
              </div>
            </VisualCard>
              ...
              <VisualCard bgClass="bg-[#EAE7FF] flex items-center justify-center">
                ...
                >
                   ...
                   <div className="flex flex-col gap-2">
                     <div className="bg-[#F9FAFB] text-gray-600 text-[13px] font-medium py-3 rounded-lg flex justify-center cursor-not-allowed opacity-70">Transcribe Manually</div>
                     <div className="bg-[#F9FAFB] text-gray-600 text-[13px] font-medium py-3 rounded-lg flex justify-center cursor-not-allowed opacity-70">Upload Subtitles</div>
                   </div>
                </motion.div>
              </VisualCard>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <VisualCard bgClass="bg-[#EBF8EF] p-0 block relative">
              <div className="absolute inset-0 w-full h-full">
                <img src="/photo/ChatGPT Image Apr 15, 2026, 10_16_13 AM.png" alt="Smart Reframe" className="w-full h-full object-contain p-8 md:p-12" />
              </div>
            </VisualCard>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
           {/* Silence Remover */}
           <motion.div whileHover={{ y: -4 }} className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-[0_8px_30px_rgba(0,0,0,0.02)] flex flex-col">
              <div className="bg-[#F4F4F5] w-12 h-12 rounded-xl flex items-center justify-center text-gray-500 mb-6"><MonitorPlay size={24}/></div>
           ...
           {/* AI Music Sync */}
           <motion.div whileHover={{ y: -4 }} className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-[0_8px_30px_rgba(0,0,0,0.02)] flex flex-col relative overflow-hidden">
              <div className="bg-[#EEF2FF] w-12 h-12 rounded-xl flex items-center justify-center text-[#5A6BFB] mb-6"><Music size={24}/></div>
              <h4 className="text-[20px] font-semibold text-[#1C1C1C] mb-3">AI Music Sync</h4>
              <p className="text-[15px] text-[#4B5563] leading-[1.6]">Cuts sync to the beat. Royalty-free music library included.</p>
              <div className="absolute -bottom-2 right-4 text-[80px] text-[#5A6BFB] opacity-5">♪</div>
           </motion.div>

           {/* Auto Color Grade */}
           <motion.div whileHover={{ y: -4 }} className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-[0_8px_30px_rgba(0,0,0,0.02)] flex flex-col">
              <div className="bg-[#FDF4FF] w-12 h-12 rounded-xl flex items-center justify-center text-pink-500 mb-6"><Palette size={24}/></div>
              ...
              <div className="mt-auto pt-6 flex items-center w-full bg-[#F4F4F5] rounded-xl p-1.5 border border-gray-200">
                 ...
                 <div className="flex-1 text-center text-[11px] font-bold text-white bg-gradient-to-r from-pink-500 to-orange-400 rounded-lg py-1.5 shadow-sm">VIVID</div>
              </div>
           </motion.div>

           {/* Brand Kit */}
           <motion.div whileHover={{ y: -4 }} className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-[0_8px_30px_rgba(0,0,0,0.02)] flex flex-col relative overflow-hidden lg:col-span-1.5">
              <div className="bg-[#F4F4F5] w-12 h-12 rounded-xl flex items-center justify-center text-gray-500 mb-6"><PenTool size={24}/></div>
              <h4 className="text-[20px] font-semibold text-[#1C1C1C] mb-3">Brand Kit</h4>
              <p className="text-[15px] text-[#4B5563] leading-[1.6]">Upload logo, set colors and fonts once. Every export is branded.</p>
              <div className="absolute -bottom-8 -right-4 text-[100px] text-gray-200 opacity-20">◈</div>
           </motion.div>

           {/* B-Roll AI */}
           <motion.div whileHover={{ y: -4 }} className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-[0_8px_30px_rgba(0,0,0,0.02)] flex flex-col relative overflow-hidden lg:col-span-1.5">
              <div className="bg-[#EEF2FF] w-12 h-12 rounded-xl flex items-center justify-center text-[#5A6BFB] mb-6"><LayoutGrid size={24}/></div>
              <h4 className="text-[20px] font-semibold text-[#1C1C1C] mb-3">B-Roll AI Suggestions</h4>
              <p className="text-[15px] text-[#4B5563] leading-[1.6]">AI matches your script with relevant stock footage from our 10M+ library.</p>
